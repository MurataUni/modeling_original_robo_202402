...
def main():
    ...
    target_name = "body_lower"

    sw = Shipwright(Dock())

    json_loader = JsonLoader(posture)
    pw = PostureWrapper(json_loader.fetch())
    del pw.postures["shoulder_base_l"], pw.postures["shoulder_base_r"]
    del pw.postures["coxa_joint_l"], pw.postures["coxa_joint_r"]
    del pw.postures["neck"]
